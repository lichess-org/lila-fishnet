...
object ChessCirceCodecs {
  ...
  given Decoder[Variant] =
    decodeString.emap(s => Variant.byName(s).toRight(s"Invalid variant: $s"))
}
...
object Fishnet {
  ...
  case class Work(id: WorkId, level: Int, clock: Option[Lila.Clock], `type`: String = "move") derives Encoder.AsObject
...
object Lila {
  ...
  case class Response(gameId: GameId, moves: String, uci: Uci) {
    def sign: String = moves.takeRight(20).replace(" ", "")
    def write: String = s"$gameId $sign ${uci.uci}"
  }
  ...
  def readMoveReq(msg: String): Either[String, Request] = {
    msg.split(";", 6) match {
      case Array(gameId, levelS, clockS, variantS, initialFenS, moves) =>
        for {
          level <- levelS.toIntOption.toRight("Invalid level")
          variant = chess.variant.Variant.orDefault(LilaKey(variantS))
          initialFen = readFen(initialFenS).getOrElse(variant.initialFen)
